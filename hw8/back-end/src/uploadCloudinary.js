...
if (!process.env.CLOUDINARY_URL) {
     process.env.CLOUDINARY_URL="cloudinary://553247615634198:rsK9amxeMc5qO_qpRV-cv-KFv3g@har8t0mso"
}
...
const doUploadWithImage = (publicName, req, res, next) => {
...
const uploadImageAndText = (publicName) => (req, res, next) => {
	if (req.body.text == null) {
		...
		multer().fields([
	    	...
	    	doUploadWithImage(publicName, req, res, next)
	    )
	} else {
		next()
	}
...
function postImage(req, res) {
   ...
   const image = `<img src=${req.fileurl}>`/*cloudinary.image(req.fileid, {
...
module.exports = { uploadImage, uploadImageAndText}
