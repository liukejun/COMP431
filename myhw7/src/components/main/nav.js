...
import {navigatePages, addError, addSuccess} from '../../action.js';
			...
			<Navbar inverse className="navbar navbar-default">
			    ...
			    <Navbar.Collapse>
			      <Nav>
			        ...
			        <NavItem id="navToProfile" eventKey={2} onClick={toProfile}>Profile</NavItem>
			      ...
			      <Nav pullRight>
			        ...
			        <NavItem eventKey={2} id="main_logout" onClick={toLanding}>Log Out</NavItem>
			      </Nav>
			    </Navbar.Collapse>
			  </Navbar>
			)
...
const Navs = connect(null, dispatch => ({
    ...
    toProfile: () => {
    	dispatch(navigatePages('Profile_Page')),
    	dispatch(addError(''))
        dispatch(addSuccess(''))
    },
    toLanding: () => {
