			...
			Profile.find({username: userid}).exec(function(err, items) {
				if (err) {
					...
					if (items.length == 0) {
						...
						// does not have that friend, add the friend to mongodb and return all new friends
						// var newFollowing = [
						// 	...curr_following,
						// 	userid
						// ]
						// Profile.update({username: username}, {$push:{following: userid} }).exec(function(err, item) {
						// 	if (err) {
						// 		console.log("error change following " + err)
						// 	} else {
						// 		console.log("change following result ")
						// 		console.log(item)
						// 		res.send({ 
						// 			username: username, 
